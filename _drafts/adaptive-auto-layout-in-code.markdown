...
The graphic below denotes the four main subviews of our hypothetical Instagram application. The challenge will be to install layout constraints in code that can be switched on and off as the [size class traits][size-class-traits] change during device rotation.
...
Our first task is to install the constraints for the generic size class, and by generic we mean constraints which apply for both regular and compact [size class traits][size-class-traits]. For Matthew's article which uses Interface Builder this corrresponds to the _any-width any-height_ setting.
...
The functions we just discussed are concerned with creating the constraints. As mentioned earlier the challenge will be to toggle constraints on and off as the [size class traits][size-class-traits] change during device rotation.
